...
class InMemoryLeaseStoreTest {
    ...
    void shouldSaveAndFindLease() {
        ...
        Optional<QuotaLease> result = leaseStore.find(quotaKey);
    ...
    void shouldReplaceExistingLeaseForSameQuota() {
        ...
        InMemoryLeaseStore leaseStore = new InMemoryLeaseStore();
        QuotaKey quotaKey = createQuotaKey();
        QuotaLease firstLease = new QuotaLease("lease-1", quotaKey, "node-1", 10, Instant.parse("2026-08-28T10:00:00Z"), Instant.parse("2026-08-28T10:01:00Z"));
        QuotaLease secondLease = new QuotaLease("lease-2", quotaKey, "node-1", 20, Instant.parse("2026-08-28T10:00:00Z"), Instant.parse("2026-08-28T10:02:00Z"));
    ...
    private QuotaKey createQuotaKey() {
        ...
        TrafficPolicy policy = new TrafficPolicy("test-policy", "Test Policy", TrafficPolicyType.TOKEN_BUCKET, PolicyStatus.ACTIVE, 100, 10, Instant.parse("2026-08-28T09:00:00Z"));
        return new QuotaKey(policy.getPolicyId(), new TrafficSubject("user-123", TrafficSubjectType.USER), "/api/orders");
    }


    private QuotaLease createLease(QuotaKey quotaKey) {
        return new QuotaLease("lease-1", quotaKey, "node-1", 10, Instant.parse("2026-08-28T10:00:00Z"), Instant.parse("2026-08-28T10:01:00Z"));
    }
}
