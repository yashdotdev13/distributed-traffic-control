...
class SlidingWindowTrafficControlAlgorithmTest {
    ...
    void shouldAllowRequestWhenCapacityIsAvailable() {
        ...
        SlidingWindowTrafficControlAlgorithm algorithm = new SlidingWindowTrafficControlAlgorithm(clock);
    ...
    void shouldRejectPolicyWithoutWindowDuration() {
        ...
        MutableClock clock = new MutableClock(START_TIME);
        SlidingWindowTrafficControlAlgorithm algorithm = new SlidingWindowTrafficControlAlgorithm(clock);
        TrafficPolicy policy = new TrafficPolicy("sliding-window-policy", "Sliding Window Policy", TrafficPolicyType.SLIDING_WINDOW, PolicyStatus.ACTIVE, 5, 1, null, START_TIME);
        Quota quota = createQuota(policy, 5, START_TIME);
    ...
    private TrafficPolicy createPolicy(long capacity, Duration windowDuration) {
        return new TrafficPolicy("sliding-window-policy", "Sliding Window Policy", TrafficPolicyType.SLIDING_WINDOW, PolicyStatus.ACTIVE, capacity, 1, windowDuration, START_TIME);
    }
    ...
    private QuotaKey createQuotaKey() {
        ...
        return new QuotaKey("sliding-window-policy", new TrafficSubject("user-123", TrafficSubjectType.USER), "/api/orders");
    }
