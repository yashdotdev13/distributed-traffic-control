...
class FixedWindowTrafficControlAlgorithmTest {
    ...
    void shouldAllowRequestWhenCapacityIsAvailable() {
        ...
        FixedWindowTrafficControlAlgorithm algorithm = new FixedWindowTrafficControlAlgorithm(clock);
        TrafficPolicy policy = createPolicy(5, Duration.ofMinutes(1));
        Quota quota = createQuota(policy, 5, START_TIME);
        QuotaConsumptionResult result = algorithm.tryConsume(quota, policy);
        assertTrue(result.isConsumed());
        assertEquals(4, result.getRemainingCapacity());
    }
    ...
    void shouldRejectRequestWhenWindowCapacityIsExhausted() {
        MutableClock clock = new MutableClock(START_TIME);
        FixedWindowTrafficControlAlgorithm algorithm = new FixedWindowTrafficControlAlgorithm(clock);
        TrafficPolicy policy = createPolicy(2, Duration.ofMinutes(1));
        Quota quota = createQuota(policy, 2, START_TIME);
    ...
    void shouldResetCapacityWhenWindowExpires() {
        ...
        MutableClock clock = new MutableClock(START_TIME);
        FixedWindowTrafficControlAlgorithm algorithm = new FixedWindowTrafficControlAlgorithm(clock);
        TrafficPolicy policy = createPolicy(3, Duration.ofMinutes(1));
        Quota quota = createQuota(policy, 3, START_TIME);
        algorithm.tryConsume(quota, policy);
        algorithm.tryConsume(quota, policy);
        algorithm.tryConsume(quota, policy);

        assertFalse(algorithm.tryConsume(quota, policy).isConsumed());
    ...
    void shouldAllowFullCapacityAfterWindowReset() {
        ...
        Quota quota = createQuota(policy, 5, START_TIME);
        for (int i = 0; i < 5; i++) {
            assertTrue(algorithm.tryConsume(quota, policy).isConsumed());
        }
        assertEquals(0, quota.getAvailableCapacity());
        clock.advanceSeconds(60);
        for (int i = 0; i < 5; i++) {
            assertTrue(algorithm.tryConsume(quota, policy).isConsumed());
        }
    ...
    void shouldRejectPolicyWithoutWindowDuration() {
        MutableClock clock = new MutableClock(START_TIME);
        FixedWindowTrafficControlAlgorithm algorithm = new FixedWindowTrafficControlAlgorithm(clock);
        TrafficPolicy policy = new TrafficPolicy("fixed-window-policy", "Fixed Window Policy", TrafficPolicyType.FIXED_WINDOW, PolicyStatus.ACTIVE, 5, 1, null, START_TIME);
        Quota quota = createQuota(policy, 5, START_TIME);
        assertThrows(NullPointerException.class, () -> algorithm.tryConsume(quota, policy));
    }

    private TrafficPolicy createPolicy(long capacity, Duration windowDuration) {
        return new TrafficPolicy("fixed-window-policy", "Fixed Window Policy", TrafficPolicyType.FIXED_WINDOW, PolicyStatus.ACTIVE, capacity, 1, windowDuration, START_TIME);
    }
    private Quota createQuota(TrafficPolicy policy, long availableCapacity, Instant createdAt) {
        return new Quota(createQuotaKey(), policy.getCapacity(), availableCapacity, createdAt);
    }

    private QuotaKey createQuotaKey() {
        return new QuotaKey("fixed-window-policy", new TrafficSubject("user-123", TrafficSubjectType.USER), "/api/orders");
    }
