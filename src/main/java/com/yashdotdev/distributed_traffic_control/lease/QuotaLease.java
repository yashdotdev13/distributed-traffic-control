...
public class QuotaLease {
    ...
    public QuotaLease(String leaseId, QuotaKey quotaKey, String nodeId, long allocatedCapacity, Instant issuedAt, Instant expiresAt) {
        ...
        if (allocatedCapacity <= 0) {
            throw new IllegalArgumentException("allocatedCapacity must be greater than zero");
        }
    ...
    public boolean canConsume(Instant currentTime) {
        return !isExpired(currentTime) && hasRemainingCapacity();
    }
    public void consume() {
        if (!hasRemainingCapacity()) {
            throw new IllegalStateException("lease has no remaining capacity");
        }
        remainingCapacity--;
    }
