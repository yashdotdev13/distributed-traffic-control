...
public class FixedWindowTrafficControlAlgorithm implements TrafficControlAlgorithm {

    private final Clock clock;

    public FixedWindowTrafficControlAlgorithm(Clock clock) {
    ...
    public QuotaConsumptionResult tryConsume(Quota quota, TrafficPolicy policy) {
        ...
        Duration windowDuration = Objects.requireNonNull(policy.getWindowDuration(), "windowDuration must not be null for FIXED_WINDOW");

        Instant now = clock.instant();
        if (isWindowExpired(quota.getWindowStartedAt(), now, windowDuration)) {
    ...
    private boolean isWindowExpired(Instant windowStartedAt, Instant now, Duration windowDuration) {
        return !now.isBefore(windowStartedAt.plus(windowDuration));
    }
}
