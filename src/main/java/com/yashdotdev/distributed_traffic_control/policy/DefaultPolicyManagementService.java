...
public class DefaultPolicyManagementService implements PolicyManagementService {
    ...
    public DefaultPolicyManagementService(InMemoryPolicyProvider policyProvider) {
    ...
    public List<PolicyRegistration> findAllPolicies() {
        return policyProvider.findAllPolicies().stream().map(entry -> new PolicyRegistration(entry.getKey(), entry.getValue())).toList();
    }

    @Override
    public TrafficPolicy updatePolicy(String resource, TrafficPolicy policy) {
        Optional<TrafficPolicy> existingPolicy = policyProvider.findPolicy(resource);
        if (existingPolicy.isEmpty()) {
            throw new IllegalArgumentException("No policy registered for resource: " + resource);
        }
        policyProvider.registerPolicy(resource, policy);
        return policy;
    }
